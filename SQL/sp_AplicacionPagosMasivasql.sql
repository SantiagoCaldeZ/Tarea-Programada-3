...
BEGIN
    ...
    BEGIN TRY
        ...
        -- 1 Pagos del dia que no han sido aplicados
        DECLARE @Pagos TABLE
        (
        ...
        INSERT INTO @Pagos
        (idPago, numeroFinca, montoPago, numeroReferencia)
    SELECT
        p.id,
        p.numeroFinca,
        ISNULL(p.monto, 0.0),
    ...
    WHERE p.fecha = @inFechaCorte
        ...
        -- 5 Monto aplicado a cada factura 
        Aplicaciones
        AS
        (
            SELECT
