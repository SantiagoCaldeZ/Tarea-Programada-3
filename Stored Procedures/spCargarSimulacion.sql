...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT
            B.baseId
            + ROW_NUMBER() OVER (ORDER BY M.fechaOperacion, M.numeroFinca, M.valorDocumento) AS id,
            M.fechaOperacion  AS fechaInicio,
            NULL              AS fechaFin,
            per.id            AS idPersona,
        ...
        -- Desasociar (tipoAsociacionId = 2)
        UPDATE pp
        SET
            pp.fechaFin = M.fechaOperacion
        FROM MovPP AS M
        JOIN dbo.Persona   AS per ON per.valorDocumento = M.valorDocumento
        JOIN dbo.Propiedad AS pr  ON pr.numeroFinca     = M.numeroFinca
        ...
        (
            SELECT
                ISNULL(MAX(cp.id), 0) AS baseId
            FROM dbo.CCPropiedad AS cp
        )
        -- Asociar CC a propiedad (vigente)
        ...
        SELECT
            B.baseId
            + ROW_NUMBER() OVER (ORDER BY M.fechaOperacion, M.numeroFinca, M.idCC) AS id,
            pr.numeroFinca AS NumeroFinca,
            M.idCC,
            pr.id          AS PropiedadId,
            M.fechaOperacion AS fechaInicio,
            NULL             AS fechaFin
        FROM MovCC AS M
        JOIN dbo.Propiedad AS pr ON pr.numeroFinca = M.numeroFinca
        CROSS JOIN BaseIdCC AS B
        ...
        -- Bitácora de eventos CCPropiedadEvento
        ...
        SELECT
            pr.id              AS idPropiedad,
            M.idCC             AS idCC,
            M.tipoAsociacionId AS idTipoAsociacion,
            M.fechaOperacion   AS fecha
        FROM MovCC AS M
        JOIN dbo.Propiedad AS pr ON pr.numeroFinca = M.numeroFinca;
